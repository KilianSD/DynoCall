...
std::wstring ConvertToWideString(const char* narrowStr){
    int numChars = MultiByteToWideChar(CP_UTF8, 0, narrowStr, -1, NULL, 0);
    std::wstring wideStr(numChars, L'\0');
    MultiByteToWideChar(CP_UTF8, 0, narrowStr, -1, &wideStr[0], numChars);
    return wideStr;
}

extern "C" {
    void EXPORT memory_injection(std::vector<void*> args){
        if(args.empty()) {
            fmt::print(fg(fmt::color::red) | fmt::emphasis::bold,
                "Error: Memory Injection requires a process name, please check your argument count.\n");
            return;
        }
